	...
	const int _truncZ)
{
	...
	Lcoeffminus = new FieldComplex3D(truncX, truncY, truncZ);
  discretelap = new FieldComplex3D(truncX, truncY, truncZ);
...
// L, K, and CD need to be initialized as zero
	void FftOper::FourierCoefficient()
	{
      ...
      for (id = 0; id < size; id++)   
      {
      	...
      	avalue = xcoeff + ycoeff + zcoeff;
      	val = pow(alpha*(xcoeff + ycoeff + zcoeff)+gamma, lpow);
      	val2 = pow(alpha*(xcoeff + ycoeff + zcoeff)+gamma, lpow-1);


      	discretelap -> data [index].real(avalue);
      	discretelap -> data [index+1].real(avalue);
      	discretelap -> data [index+2].real(avalue);

      	Lcoeff->data[index].real(val); 
      	Lcoeff->data[index+1].real(val);
      	Lcoeff->data[index+2].real(val);

      	Lcoeffminus -> data[index].real(val2); 
      	Lcoeffminus -> data[index+1].real(val2);
      	Lcoeffminus -> data[index+2].real(val2);
